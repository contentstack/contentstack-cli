        ...
        function() {
          ...
          helper.mkdir(path + '/themes/' + skeleton +
            ...
            function() {
              ...
              setTimeout(function() {
                console.log(
                  '\n\x1b[36mRunning \x1b[33mnpm install \x1b[36min the directory \x1b[33m%s \x1b[36m[%s] to install the required dependencies. If this fails, try running the command yourself.',
                  path, _chdir);
                ...
                exec('npm install', function(error, stdout,
                  ...
                  for (var i = 0, _i = nextSteps.length; i <
                    _i; i++) {
                    console.log('\n\x1b[36m  %d. %s', (i +
                      1), nextSteps[i]['info']);
...
Connect.prototype.init = function() {
  ...
  _requests = [
    ...
    function(environments, _cb) {
      if (environments && environments.length) {
        ...
        for (var i = 0, _i = environmentsLength; i < _i; i++) {
          if (environments[i]['servers'] && environments[i]['servers'].length ==
            1) {
            _message +=
        ...
        prompt.get([
          ...
          inputs.selected_environment = environments[parseInt(inputs.environment_select) -
            1]['name'];
          inputs.environments = environments;
          return _cb(null, 1);
        });
      } else {
        console.info('\x1b[33m\nNo publishing environment exists on %s.',
          helper.url, 'Please create one.');
        return _cb(null, 2);
      }
    },
    ...
    function(result, _cb) {
      switch (result) {
        case 1:
          var _servers = inputs.environments[parseInt(inputs.environment_select) -
            1]['servers'];
          if (_servers && _servers.length > 1) {
            var _serverMessage =
              `\x1b[36mThe environment \'${inputs.selected_environment}\' has multiple servers.\nPlease select one.`;
            ...
            prompt.get([
              ...
              inputs.selected_server = _servers[parseInt(inputs.environment_server_select) -
                1]['name'];
              return _cb(null);
            });
          } else {
...
Connect.prototype.getDirectory = function(done) {
  ...
  function checkDirectory() {
    helper.emptyDirectory(inputs.directory, function(error, empty) {
      if (error) {
        log(error(error));
