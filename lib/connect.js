...
var sampleCSS = [
    ...
    , 'img{background:0 0;border:10px solid rgba(0,0,0,.12);border-radius:4px;display:block;margin:1.3em auto;max-width:95%}'
].join(eol);
        ...
        function (_cb) {
            request({
                    ...
                    json: {
                        ...
                        query : {
                            only: {
                                BASE: ["name", "servers", "deploy_content"]
                            },
                            query: {
                                deploy_content: true
                            }
...
Connect.prototype.stackInput = function (done) {
    ...
    function checkSite() {
        ...
        request({
            ...
            .then(function (body) {
                if (body && body.stack && body.stack.discrete_variables && body.stack.discrete_variables.access_token && body.stack.discrete_variables.access_token == self.headers.access_token) {
    ...
    if(!(self.options.api_key && self.options.access_token)){
        prompt.get([
                ...
                if (!err && result) {
                    _.merge(inputs, result);
                    self.headers.api_key = inputs.api_key;
                    self.headers.access_token = inputs.access_token;
                    checkSite();
                } else {
                    done(err);
                }
        ...
        self.headers.api_key = inputs.api_key = self.options.api_key;
